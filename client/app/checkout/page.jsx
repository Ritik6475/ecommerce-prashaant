...
export default function CheckoutPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    confirmPhone: '', street: '', city: '', state: '', postalCode: '', country: 'India'
  });
  ...
  const handlePayment = async (e) => {
    ...
    try {
      const orderItems = items.map(item => ({
        product: item.product._id, size: item.size, quantity: item.quantity, price: item.product.offerprice,
      color: item.color ?? item.selectedColor ?? item.variant?.color ?? null, // <-- include color
      ...
      const { data: orderData } = await axios.post('/orders', {
        items: orderItems, totalAmount: total, address: formData, deliveryOption, orderNotes
      });

      const { data: paymentData } = await axios.post('/payment/create-order', {
        amount: total, orderId: orderData.order._id
      });
              ...
              <div className="p-4">
                <div className="space-y-3">
                  <div className="p-3 border border-gray-900 rounded-md bg-gray-50">
                    <div className="flex items-center">
                      <input id="razorpay" name="payment-method" type="radio" defaultChecked className="h-4 w-4 text-gray-900" />
                  ...
                  <div className="p-3 border border-gray-200 rounded-md">
                    <div className="flex items-center">
                      <input id="cod" name="payment-method" type="radio" className="h-4 w-4 text-gray-900" />
